...
export default function PenpenAdminTournamentsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [leagueResponse, tournamentResponse] = await Promise.all([
          penpenAdminMutate<
            {
              id: string;
              name: string;
        ...
        const leagues = (leagueResponse.data ?? [])
          .filter((item) => item.id !== DEFAULT_LEAGUE_ID)
          .map((item) => ({
            id: item.id,
            name: item.name,
        ...
        const loaded = normalizeTournaments(
          (tournamentResponse.data ?? [])
            .filter((item) => item.league_id !== DEFAULT_LEAGUE_ID)
            .map((item) => ({
              id: item.id,
        ...
        window.alert(
          `トーナメントデータの取得に失敗しました: ${error instanceof Error ? error.message : "unknown"}`,
        );
      }
    };
